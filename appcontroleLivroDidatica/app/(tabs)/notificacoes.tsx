...
interface Notificacao {
  ...
  data: string;
  lida: boolean;
  destinatarios: string; // "alunos", "professores", "todos"
}
...
export default function NotificacoesScreen() {
  ...
  const getDestinatariosTexto = (destinatarios: string) => {
  ...
  const renderItem = ({ item }: { item: Notificacao }) => (
    <View style={[styles.card, item.lida ? styles.cardLida : styles.cardNaoLida]}>
      <View style={styles.cardContent}>
        <Text style={styles.cardTitle}>{item.titulo}</Text>
        <Text style={styles.cardSubtitle}>{item.mensagem}</Text>
        <Text style={styles.cardInfo}>
          Data: {item.data} | Para: {getDestinatariosTexto(item.destinatarios)}
        </Text>
      </View>
      <View style={styles.cardActions}>
        <TouchableOpacity onPress={() => marcarComoLida(item.id)}>
          <MaterialIcons 
            name={item.lida ? "mark-email-read" : "mark-email-unread"} 
            size={24} 
            color={item.lida ? "#4CAF50" : "#2196F3"} 
          />
        </TouchableOpacity>
          ...
          <ScrollView style={styles.modalContent}>
            ...
            <View style={styles.radioGroup}>
              ...
              >
                <View style={[
                  styles.radioCircle, 
                  destinatarios === 'todos' && styles.radioSelected
                ]} />
              ...
              <TouchableOpacity 
                style={styles.radioOption} 
                onPress={() => setDestinatarios('alunos')}
              >
                <View style={[
                  styles.radioCircle, 
                  destinatarios === 'alunos' && styles.radioSelected
                ]} />
              ...
              <TouchableOpacity 
                style={styles.radioOption} 
                onPress={() => setDestinatarios('professores')}
              >
                <View style={[
                  styles.radioCircle, 
                  destinatarios === 'professores' && styles.radioSelected
                ]} />
